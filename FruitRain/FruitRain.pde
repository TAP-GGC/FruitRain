 ...
 */
void draw() {
  // Display startup screen animation
  ...
  if (!start) {
    ...
    quizLoaded = false;
    streak = 0;
    displayInst(); // Display instructions if the game hasn't started yet
  }
  ...
  if (start) {
    ...
    // If a quiz hasn't been chosen yet
    if (!quizChosen) {
      fill(RASPBERRY);
      textSize(40); // Set text size for the quiz selection
      ...
      // If a quiz has been chosen but not loaded yet
      if (quizChosen && !quizLoaded) {
        questions = loadQuestions(); // Load the quiz questions
        quizSize = questions.size(); // Set the quiz size
        quizLoaded = true;
        musicSwap(); // Swap the music track
      }

      // Handle different scenarios
      if (scenario == 2) {
        delay(3000); // Delay for correct answer
      }
      if (scenario == 3) {
        delay(1500); // Delay for incorrect answer
      }
      if (scenario == 4) {
        start = false; // Game over, reset start flag
        delay(5000); // Delay before restarting
      }

      // Reset scenario if not 1
      if (scenario != 1) {
        scenario = 1;
        guess = 5;
        delay(500);
      ...
      if (scenario == 1) {
        try {
          ...
          if ((quizIndex) % 6 != 0 || quizIndex == 0) {
            ...
            if (!oneUp) {
              healthPoints++; // Award extra life every 6 questions
              oneUp = true; // Ensure it's awarded only once
            }
          }

          fill(ORANGE);
          text("Lives left: " + healthPoints, 5, 50); // Display remaining lives
          ...
          for (int j = 1; (j - 1) < questions.get(quizIndex).getAnswers().size(); j++) {
            enemies[j - 1].drawEnemy(); // Draw the enemy (fruit)
            fill(color(255, 0, 46)); // Set color for answers
            text(questions.get(quizIndex).getAnswers().get(j - 1), (enemies[j - 1].getEnemyX() + 50), (enemies[j - 1].getEnemyY() + 75)); // Display the answers
          }

          guess = 5; // Reset guess
        } catch (IndexOutOfBoundsException ex) {
          scenario = 4; // Quiz is over, trigger game over scenario
          System.out.println("The quiz is over");
          fill(RASPBERRY);
          text("You win! Final Score: " + score, 450, 450); // Display winning message
        }
      }
    }
  }
